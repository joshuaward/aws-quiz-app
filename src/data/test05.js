...
const questions = [ // Domain 1 – 6 questions (AWS services focus)
    ...
    // Domain 2 – 7 questions (AWS services + GenAI concepts)
    {
        ...
        options: [
            "A training cluster for fine-tuning Claude models from scratch",
            "A dedicated GPU cluster the developer must configure",
            "A fully managed API that abstracts all underlying infrastructure for foundation model inference",
            "A self-hosted Docker container the developer must maintain"
        ],
        correct: [2],
    ...
    },
    {
        ...
        options: [
            "Amazon Bedrock only supports Amazon-developed models",
            "Amazon Bedrock models always produce better outputs than self-hosted models",
            "Amazon Bedrock eliminates the need to manage infrastructure, patching, and scaling — reducing operational overhead and accelerating time to market",
            "Self-hosted models are always more expensive than Amazon Bedrock"
        ],
        correct: [2],
    ...
    },
    {
        ...
        options: [
            ...
            "Large language model (LLM) with code generation",
            "Text-to-image generative model (e.g., Stability AI Stable Diffusion on Bedrock)",
    ...
    // Domain 3 – 9 questions (services + application design)
    {
        ...
        options: [
            "Retrieve documents → Fine-tune model → Generate response → Return to user",
            "Generate response → Retrieve documents → Embed query → Return to user",
            "Embed query → Retrieve relevant documents from vector store → Augment prompt with documents → Generate response",
            "Fine-tune model → Deploy model → Retrieve documents → Generate response"
        ],
        correct: [2],
    ...
    },
    {
        ...
        options: [
            ...
            "Full pre-training from scratch on company data",
            "Instruction fine-tuning on the 200 conversation examples to teach response style",
            "Continuous pre-training on all company documentation"
        ],
        correct: [2],
    ...
    },
    {
        ...
        options: [
            "To optimize hyperparameters for fairness",
            "To retrain the model with balanced weights",
            "To identify bias in the training data BEFORE model training begins, allowing teams to remediate data issues proactively",
            "To detect model drift in production"
        ],
        correct: [2],

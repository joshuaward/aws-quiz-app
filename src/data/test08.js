...
const questions = [{
    ...
    },
    {
        ...
        options: [
            "The model should immediately be discarded due to low recall",
            "The model has a high false positive rate",
            "The model is conservative — it rarely makes positive predictions it doesn't believe, but misses many actual positives",
            "High precision (0.85) means when the model predicts positive, it is usually correct",
            "An F1 score of 0.71 indicates the model is performing well on all classes equally"
        ],
        correct: [2, 3],
    ...
    },
    {
        ...
        options: [
            "Cross-validation is a technique for cleaning missing values in a dataset",
            "Cross-validation trains the same model multiple times with different hyperparameters to find the best configuration",
            "Cross-validation provides a more robust performance estimate by training and evaluating on multiple different data splits, reducing the variance of a single train-test split",
            "Cross-validation is used exclusively for deep learning models"
        ],
        correct: [2],
    ...
    },
    {
        ...
        options: [
            "When the deployment requires extremely low inference latency",
            "When the task requires memorizing 10,000 proprietary product SKU numbers",
            "When the task is general text formatting that can be demonstrated with 3–5 clear examples",
            "When the model needs to learn specialized scientific vocabulary"
        ],
        correct: [2],
    ...
    },
    {
        ...
        options: [
            "Foundation models only work for text generation and cannot handle code or translation",
            "Foundation models are purpose-trained for each of these specific tasks separately",
            "Foundation models develop broad general capabilities during pre-training that can be directed toward multiple different tasks via prompting or fine-tuning",
            "Foundation models require a separate API endpoint for each task"
        ],
        correct: [2],
    ...
    },
    {
        ...
        options: [
            "It resets the conversation history to start fresh",
            "It pauses model inference after a set time limit",
            "It specifies a token or string that, when generated, causes the model to stop producing output",
            "It defines the maximum number of tokens the model can generate"
        ],
        correct: [2],
